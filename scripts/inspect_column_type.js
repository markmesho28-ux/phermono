...
async function run() {
  try {
    const { data, error } = await supabase
      ...
      .eq('column_name', 'id');
    if (error) console.error('Error querying information_schema:', error.message || error);
    console.log('information_schema.columns for subcategories.id:', JSON.stringify(data, null, 2));
