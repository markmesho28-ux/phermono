...
export function CartDrawer({ isOpen, onClose, cartItems, onUpdateQty, onRemove, onCheckout }: CartDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer Panel */}
      <aside
        className={`fixed top-0 right-0 h-full w-full sm:w-[440px] bg-white z-50 flex flex-col shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-brand-gold-border/40 bg-brand-cream/60">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full hover:bg-stone-200 text-stone-500 hover:text-brand-black transition-colors"
        ...
        {/* Free Shipping Meter */}
        <div className="bg-brand-gold-light px-6 py-3.5 border-b border-brand-gold-border/60">
          <div className="flex items-center justify-between text-xs font-semibold text-brand-black mb-1.5">
            <span>
              {remaining === 0 ? (
                <span className="text-emerald-700 flex items-center gap-1">
                  <Sparkles size={13} /> You unlocked Free Express Courier!
                </span>
              ) : (
                <span>Add ${remaining.toFixed(2)} more for Free Shipping</span>
              )}
            </span>
            <span className="text-[10px] text-stone-500">{Math.round(progress)}%</span>
          </div>
        ...
        {/* Footer Checkout */}
        {cartItems.length > 0 && (
          <div className="p-6 border-t border-brand-gold-border/40 bg-brand-cream/80 space-y-4">
            <div className="space-y-1.5 text-sm">
              ...
              <div className="border-t border-stone-200 pt-2 flex justify-between text-base font-bold text-brand-black">
                ...
                <span>${(subtotal + (remaining === 0 ? 0 : 4.99)).toFixed(2)}</span>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <div className="flex items-center bg-white border border-stone-200 rounded-full px-2 py-0.5 shadow-inner">
            <button
              ...
              className="p-1 text-stone-500 hover:text-brand-black"
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden pointer-events-auto border border-brand-gold-border/60 animate-slideUp flex flex-col md:flex-row">
          ...
          <div className="md:w-1/2 relative bg-brand-sand/40 p-6 flex items-center justify-center">
            ...
            />
            {product.tag && (
              <span className="absolute top-4 left-4 bg-brand-black text-brand-gold font-bold text-[10px] uppercase px-3 py-1 rounded-full shadow-sm">
                {product.tag}
              </span>
            )}
            {discount && (
